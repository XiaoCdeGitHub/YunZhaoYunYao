...
// Pass this if window is not defined yet
  ...
  const $ = {}
  const style = '#__previewImage-container{-ms-touch-action:none;touch-action:none;-webkit-touch-action:none;line-height:100%;background-color:#000;width:100%;height:100%;position:fixed;overflow:hidden;top:0;left:0;z-index: 2147483647;transition:transform .3s;-ms-transition:transform .3s;-moz-transition:transform .3s;-webkit-transition:transform .3s;-o-transition:transform .3s;transform:translate3d(100%,0,0);-webkit-transform:translate3d(100%,0,0);-ms-transform:translate3d(100%,0,0);-o-transform:translate3d(100%,0,0);-moz-transform:translate3d(100%,0,0)}#__previewImage-container .previewImage-text{position:absolute;top:.6em;text-align:center;font-size:18px;line-height:25px;color:#fff;z-index:10;padding: 0.2em 0.4em;background-color: rgba(255,255,255,0.4);border-radius: 50%;letter-spacing: 0;right:.8em}#__previewImage-container .previewImage-text .previewImage-text-index{font-size: 24px;}#__previewImage-container .previewImage-box{width:100%;height:100%}#__previewImage-container .previewImage-box .previewImage-item{width:100%;float:left;text-align:center;background:url(http://static.luyanghui.com/svg/oval.svg) no-repeat center/auto}#__previewImage-container .previewImage-box .previewImage-item.previewImage-nobackground{background:none}#__previewImage-container .previewImage-box .previewImage-item .previewImage-image{vertical-align:middle;width:100%;background-color: #fff;}'
  ...
  const _previewImage = function () {
    ...
    const $style = document.createElement('style') // 样式标签
  ...
  _previewImage.prototype.start = function (obj) { // 可优化 todo
    const urls = obj.urls // 待预览的图片列表
    const current = obj.current // 当前预览的图片地址
  ...
  _previewImage.prototype.render = function () {
    ...
    if (this.$box === false) { // 加载图片容器
      ...
      box.className += 'previewImage-box f-c-c-c'
      this.$box = box // 更新图片容器
    }
    else {
      this.$box.innerHTML = '' // 已有图片容器-清空容器
    }
    const text = document.createElement('div') // 当前张数/总张数--文本标签
    this.$text = text
    ...
    this.urls.forEach((v, i) => { // 图片
      const div = document.createElement('div')
      const hash = window.md5 ? md5(v + i) : v + i
      let img
      const imgCache = _this.imgLoadCache[hash]
      // 缓存图片&&读取缓存图片
      ...
      _this.imgStatusCache[i] = { hash, x: 0, m: 0, y: 0, my: 0, scale: _this.scale, scalem: 1 } // 修改缓存状态
    ...
    const offsetX = -this.imageChageMoveX * this.index // 计算显示当前图片，容器所需偏移量
  ...
  _previewImage.prototype.bind = function () {
    ...
    const touchStartFun = function () {
      _this.touchStartFun.call(_this)
    }
    const touchMoveFun = function () {
      _this.touchMoveFun.call(_this)
    }
    const touchEndFun = function () {
    ...
    const reSizeFun = function () {
      ...
      _this.$container.style.height = `${_this.winh}px` // 改变高度
      _this.imageChageMoveX = _this.marginRight + _this.winw
      const offsetX = -_this.imageChageMoveX * _this.index // 计算显示当前图片，容器所需偏移量
      try {
        _this.box.x = offsetX // 将图片容器所需偏移量，存入状态缓存器
        _this.translateScale(_this.bIndex, 0)
      }
  ...
  _previewImage.prototype.moveAction = function (ts, tm) {
    ...
    const imgStatus = this.getIndexImage()
    const maxWidth = this.winw * 0.3 / imgStatus.scale
    const x0_offset = tm.x0 - ts.x0
    const y0_offset = tm.y0 - ts.y0
    if (Math.abs(y0_offset) > 0) { // 阻止Ios系统的浏览器环境上下滑动
      event.preventDefault()
    }
    const imgPositionX = imgStatus.x + x0_offset
    const imgPositionY = imgStatus.y + y0_offset
    const allow = this.getAllow(this.index)
    const allowX = this.allowX = allow.x
    let allowY = this.allowY = allow.y0
    if (x0_offset <= 0) { // 边界
      this.allowX = -allowX
    }
    if (y0_offset <= 0) { // 边界
    ...
    if (tm.length == 1) { // 单手指(图片位移)
      if (imgStatus.scale > 1) {
        ...
        if (Math.abs(y0_offset) > 5 && this.statusX != 5) { // 长图片处理
          const $img = this.getJqElem(this.index)
          const imgBottom = $img.height - this.winh
      ...
      let scalem = this.getScale(ts, tm)
      let scale = scalem * imgStatus.scale
      if (scale >= this.maxScale) { // 达到最大放大倍数
        const over = scale - this.maxScale
  ...
  _previewImage.prototype.endAction = function (ts, te) {
    const imgStatus = this.getIndexImage()
    const x0_offset = te.x0 - ts.x0
    const y0_offset = te.y0 - ts.y0
    const time = te.time - ts.time
    let slipTime = 0
    ...
    if (ts.length == 1) { // 单手指(图片位移)
      ...
      const scale = imgStatus.scale * imgStatus.scalem
      const $img = this.getJqElem(this.index)
  ...
  _previewImage.prototype.getAllow = function (index) {
    const $img = this.getJqElem(index)
    const imgStatus = this.getIndexImage(index)
    const allowX = Math.floor(($img.width * imgStatus.scale - this.winw) / (2 * imgStatus.scale))
    let allowY0, allowY1
    if ($img.height * imgStatus.scale <= this.winh) {
      allowY0 = 0
      allowY1 = 0
  ...
  _previewImage.prototype.getScale = function (ts, tm) {
    const fingerRangeS = Math.sqrt((ts.x1 - ts.x0) ** 2 + (ts.y1 - ts.y0) ** 2) // 两手指的初始距离
    const fingerRangeM = Math.sqrt((tm.x1 - tm.x0) ** 2 + (tm.y1 - tm.y0) ** 2) // 两手指移动过程中的距离
    const range = fingerRangeM / fingerRangeS
    return range
  }

  _previewImage.prototype.imgStatusRewrite = function (index) {
    var index = index === undefined ? this.index : index
    const imgStatus = this.imgStatusCache[index]
    imgStatus.x = 0
    imgStatus.y = 0
    imgStatus.m = 0
    imgStatus.my = 0
  ...
  _previewImage.prototype.changeIndex = function (x) {
    ...
    if (this.index == 0 && x == -1) {
      ...
      this.index += x
      this.$text.innerHTML = `<span class='previewImage-text-index'>${this.index + 1}/</span>${this.maxLen + 1}` // 当前第几张/图片总数
      const hash = this.imgStatusCache[this.index].hash
      const imgCache = this.imgLoadCache[hash]
  ...
  _previewImage.prototype.translateScale = function (index, duration) {
    ...
    const scale = imgStatus.scale * imgStatus.scalem
    const offsetX = imgStatus.x + imgStatus.m
    const offsetY = imgStatus.y + imgStatus.my
    const tran_3d = `scale3d(${scale},${scale},1) ` + ` translate3d(${offsetX}px,${offsetY}px,0px)`
    // var tran_2d='scale('+scale+','+scale+') '+' translate(' + offsetX + 'px,' + offsetY +'px)';
    const transition = `transform ${duration}s ease-out`
  ...
  _previewImage.prototype.getJqElem = function (index) {
    ...
    if (index <= this.maxLen) {
      const hash = this.imgStatusCache[index].hash
      const img = this.imgLoadCache[hash].elem
      $elem = img
    }
    else {
      const elem = this.imgStatusCache[index].elem
      $elem = elem
    }
  ...
  _previewImage.prototype.getTouches = function (e) {
    ...
    const obj = { touches, length: touches.length }
